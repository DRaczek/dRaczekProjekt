    ...
    */
    function routeRequest($requestUri, $routes) {
        ...
        foreach ($routes as $route => $params) {
            ...
            $pattern = str_replace(":pageableUsers", '\?((page=(\d)+)(&size=(\d)+)((&email=([A-Za-z\d])+)*)((&firstName=([\p{L}\p{N}\s])+)*)((&lastName=([\p{L}\p{N}\s])+)*)((&status=(\d)+)*)((&createdDate=([\d-])+)*)((&id=(\d)+)*)((&orderBy=([A-Za-z_])+)*)((&order=([A-Za-z])+)*))', $pattern);
            ...
            $pattern = str_replace(":pageableCategories", '\?((page=(\d)+)(&size=(\d)+)((&name=([\p{L}\p{N}\s])+)*)((&status=(\d)+)*)((&createdDate=([\d-])+)*)((&id=(\d)+)*)((&orderBy=([A-Za-z_])+)*)((&order=([A-Za-z])+)*))', $pattern);
            $pattern = str_replace(":pageableProducts", '\?((page=(\d)+)(&size=(\d)+)((&name=([\p{L}\p{N}\s])+)*)((&categoryId=(\d)+)*)((&description=([\p{L}\p{N}\s.,:;"\'() \n-])+)*)((&priceFrom=(\d+(?:\.\d+)?))*)((&priceTo=(\d+(?:\.\d+)?))*)((&quantityFrom=(\d)+)*)((&quantityTo=(\d)+)*)((&productSize=(\d)+)*)((&colour=(\d)+)*)((&gender=(\d)+)*)((&viewCountFrom=(\d)+)*)((&viewCountTo=(\d)+)*)((&status=(\d)+)*)((&createdDate=([\d-])+)*)((&id=(\d)+)*)((&orderBy=([A-Za-z_])+)*)((&order=([A-Za-z])+)*))', $pattern);
            $pattern = str_replace(":pageableOrders", '\?((page=(\d)+)(&size=(\d)+)((&first_name=([\p{L}\p{N}\s])+)*)((&last_name=([\p{L}\p{N}\s])+)*)((&street=([\p{L}\p{N}\s])+)*)((&postal_code=([-\d])+)*)((&postal_city=([\p{L}\p{N}\s])+)*)((&country=([\p{L}\p{N}\s])+)*)((&nip=([\d])+)*)((&company_name=([\p{L}\p{N}\s])+)*)((&delivery_id=(\d)+)*)((&payment_method_id=(\d)+)*)((&payment_status=(\d)+)*)((&order_status=(\d)+)*)((&status=(\d)+)*)((&createdDate=([\d-])+)*)((&id=(\d)+)*)((&orderBy=([A-Za-z_])+)*)((&order=([A-Za-z])+)*))', $pattern);
